...
async def root():
    """Root endpoint."""
    return {"service": "MCP Service", "version": "1.0.0", "status": "running"}
...
async def health():
    ...
    return {"status": "healthy", "servers": server_status}
...
async def restart_servers():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to restart servers: {str(e)}"
        )
