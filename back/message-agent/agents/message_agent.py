...
class MessageAgent:
    ...
    def __init__(self, gemini_api_key: Optional[str] = None):
        ...
        if not self.gemini_api_key:
            logger.warning(
                "No Gemini API key provided. Agent will work with limited functionality."
            )
            self.llm = None
        else:
    ...
    def _build_workflow(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            ...
            {"search": "search_messages", "error": "handle_error"},
        )
    ...
    async def _parse_query(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            logger.info(
                f"Parsed action: {state['agent_action']}, terms: {state['search_terms']}"
            )
            ...
            if action == "recent":
                ...
                try:
                    ...
                    state["search_results"] = (
                        [msg.__dict__ for msg in messages] if messages else []
                    )
                except Exception as e:
                ...
                try:
                    ...
                    logger.info(
                        f"Search query: '{search_query}' found {len(state.get('search_results', []))} results"
                    )

                except Exception as e:
                    logger.warning(
                        f"Search service not available, using mock data: {e}"
                    )
                    state["search_results"] = self._generate_mock_results(query, action)
    ...
    def _retrieve_context(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if not search_results:
                state["context"] = (
                    "No relevant messages found in the conversation history."
                )
                return state
        ...
        try:
            ...
            prompt = ChatPromptTemplate.from_messages(
                [
                    (
                        "human",
                        """You are a helpful assistant that helps users find and understand messages from their conversation history.
...
User Query: {query}""",
            ...
            if self.llm:
                ...
                if not can_proceed:
                    logger.warning(
                        "Gemini daily quota exceeded, using fallback response"
                    )
                    response = self._generate_fallback_response(state)
                else:
                    try:
                        ...
                        if (
                            ...
                            logger.warning(
                                f"⚠️ Rate limit hit despite precautions, falling back: {llm_error}"
                            )
                            response = self._generate_fallback_response(state)
                        else:
