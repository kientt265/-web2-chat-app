...
class ServiceDiscoveryClient:
    ...
    async def register_service(self, registration: ServiceRegistration) -> str:
        ...
        response = await self._client.post(
            f"{self.registry_url}/api/v1/register", json=registration.model_dump()
        )
    ...
    async def discover_services(
        ...
        response = await self._client.post(
            f"{self.registry_url}/api/v1/discover", json=query.model_dump()
        )
        response.raise_for_status()
    ...
    async def get_healthy_service_url(
        ...
        services = await self.discover_services(
            service_type=service_type, name=name, status=ServiceStatus.HEALTHY
        )
