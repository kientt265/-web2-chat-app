...
class HTTPMCPClient:
    ...
    def __init__(self, mcp_service_url: str = None):
        ...
        """
        self.mcp_service_url = mcp_service_url or os.getenv(
            "MCP_SERVICE_URL", "http://mcp-service:3005"
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{self.mcp_service_url}/servers/{server_name}/tools"
                )
                response.raise_for_status()
                ...
                logger.debug(
                    f"Retrieved {len(tools)} tools from server '{server_name}'"
                )
                return tools
